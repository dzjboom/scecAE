...
def get_dict_mnn(data_matrix, batch_index, k=5, save=True, approx=True,approx_method="hnswlib", verbose=False, return_distance=False,metric="cosine",flag="in",log=None):
    ...
    if(flag=="in"):
        ...
        if(verbose):
            log.info("scecAe finds ({}) KNN pairs in dataset finally".format(num_KNN))
        #print("done")        
        if not return_distance:
            return mnns
        else:
        ...
        if(verbose):
            log.info("scecAe finds ({}) MNN pairs in dataset finally".format(num_MNN))
        #print("done")
        if not return_distance:
            return mnns
        else:
...
def get_dict_mnn_para(data_matrix, batch_index, k=5, save=True, approx=True,approx_method="hnswlib", verbose=False, return_distance=False,metric="cosine",flag="in",njob=8,log=None):
    ...
    if(flag=="in"):
        ...
        if(verbose):
            log.info("scecAe finds ({}) KNN pairs in dataset finally".format(len(mnns)))
        #print("done")        
        if not return_distance:
            return mnns
        else:
        ...
        mnns=list(itertools.chain(*res))
        if(verbose):
            log.info("scecAe finds ({}) MNN pairs in dataset finally".format(len(mnns)))
        #print("done")
        if not return_distance:
            return mnns
        else:
